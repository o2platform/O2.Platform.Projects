...
namespace ICSharpCode.SharpDevelop.Dom
{
	...
	public class IntegerLiteralResolveResult : ResolveResult
	{
		...
		public override ArrayList GetCompletionData(IProjectContent projectContent)
        {
            //DC added (original returned null)
            //IntegerLiteralResolveResult(callingClass, callingMember, projectContent.SystemTypes.Int32);
            var completionData = base.GetCompletionData(projectContent);
            var result = new ArrayList();
            try
            {
                foreach (DefaultMethod method in completionData)
                {
                    if (method.DeclaringType.FullyQualifiedName == this.ResolvedType.FullyQualifiedName)
                        result.Add(method);
                    else
                    {
                        if (method.Parameters.Count > 0)
                        {                            
                            if (method.TypeParameters.Count ==0)        //for now ignore all generics since the constaints are not currently being taken into account
                                result.Add(method);                                
                        }
                    }
