...
namespace FluentSharp.XObjects
{
    ...
    {        
        ...
        public static Assembly xsdCreateAssembly(this string xsdFile, string targetAssembly)
        {
            ...
            {
                var assembly = csFile.compileSourceFile();
                if (assembly.notNull())
                    assembly.Location.file_Copy(targetAssembly);
            }
        ...
        public static List<Type> xsdCsFileGetRootTypes(this string csFile)
        {
            var assembly = (csFile.fileExists())
                                ? csFile.compileSourceFile()
                                : csFile.save().compileSourceFile();
