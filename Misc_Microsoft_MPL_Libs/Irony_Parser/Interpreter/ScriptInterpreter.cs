...
namespace Irony.Interpreter {
  ...
  public class ScriptInterpreter {
    ...
    private void CheckNotBusy() {
      if (IsBusy())
        throw new Exception(O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrInterpreterIsBusy);
    }
    ...
    private IInterpretedAstNode GetAstInterface()  {
        Check(ParsedScript != null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrParseTreeNull);
        Check(ParsedScript.Root != null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrParseTreeRootNull);
        var astNode = ParsedScript.Root.AstNode;
        Check(astNode != null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrRootAstNodeNull);
        var iInterpNode = astNode as IInterpretedAstNode;
        Check(iInterpNode != null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrRootAstNoInterface);
        return iInterpNode;
    }
      ...
      if (ParsedScript.HasErrors()) {
        ...
        if (PrintParseErrors) {
          foreach(var err in ParsedScript.ParserMessages) {
            var msg = string.Format(O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrOutErrorPrintFormat, err.Location.ToUiString(),  err.Message);
