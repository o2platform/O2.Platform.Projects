...
namespace Irony.Parsing.Construction { 
  ...
  internal class GrammarDataBuilder {
    ...
    private void CollectTermsRecursive(BnfTerm term) {
      ...
      if (nt.Rule == null)
        _language.Errors.AddAndThrow(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrNtRuleIsNull, nt.Name);
      //check all child elements
      foreach (BnfTermList elemList in nt.Rule.Data)
        for (int i = 0; i < elemList.Count; i++) {
          BnfTerm child = elemList[i];
          if (child == null) {
            _language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrRuleContainsNull, nt.Name, i);
            continue; //for i loop 
          }
      ...
      foreach(var nt in _grammarData.NonTerminals) {
        ...
        if(nt.FlagIsSet(TermFlags.IsTransient)) {
          ...
          if (nt.FlagIsSet(TermFlags.IsList))
            _language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrListCannotBeTransient, nt.Name);
        ...
        foreach(var prod in nt.Productions)
          if(prod.IsSet(ProductionFlags.IsError)) {
            ...
            if (!(lastTerm is Terminal) || lastTerm == _grammar.SyntaxError)
              _language.Errors.Add(GrammarErrorLevel.Warning, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrLastTermOfErrorProd, nt.Name);
                // "The last term of error production must be a terminal. NonTerminal: {0}"
          }//foreach prod
      }//foreac nt

      if (missingAstTypeSet.Count > 0)
        _language.Errors.Add(GrammarErrorLevel.Warning, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrNodeTypeNotSetOn, missingAstTypeSet.ToString());
      if (invalidTransSet.Count > 0)
        _language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrTransientNtMustHaveOneTerm,invalidTransSet.ToString());
    }//method
