  ...
  public class StringLiteral : CompoundTerminalBase {
    ...
    public override void Init(GrammarData grammarData) {
      ...
      if (_subtypes.Count == 0) {
        grammarData.Language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrInvStrDef, this.Name); //"Error in string literal [{0}]: No start/end symbols specified."
        return; 
      }
      ...
      foreach (StringSubType subType in _subtypes) {
        if (allStartSymbols.Contains(subType.Start))
          grammarData.Language.Errors.Add(GrammarErrorLevel.Error, null, 
            O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrDupStartSymbolStr, subType.Start, this.Name); //"Duplicate start symbol {0} in string literal [{1}]."
      ...
      if(isTemplate) {
        ...
        if(templateSettings == null)
          grammarData.Language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrTemplNoSettings, this.Name); //"Error in string literal [{0}]: IsTemplate flag is set, but TemplateSettings is not provided."
        else if (templateSettings.ExpressionRoot == null)
            grammarData.Language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrTemplMissingExprRoot, this.Name); //""
        else if(!Grammar.SnippetRoots.Contains(templateSettings.ExpressionRoot))
            grammarData.Language.Errors.Add(GrammarErrorLevel.Error, null, O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrTemplExprNotRoot, this.Name); //""
      }//if
      //Create editor info
    ...
    private bool CompleteReadBody(ISourceStream source, CompoundTokenDetails details) {
      ...
      while (true) {
        ...
        if (malformed) {
          ...
          details.Error = O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrBadStrLiteral;//    "Mal-formed  string literal - cannot find termination symbol.";
    ...
    protected override bool ConvertValue(CompoundTokenDetails details) {
      ...
      if (details.IsSet((short)StringOptions.IsChar)) {
        if (value.Length != 1) {
          details.Error = O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrBadChar;  //"Invalid length of char literal - should be a single character.";
          return false;
        }
    ...
    protected virtual string HandleSpecialEscape(string segment, CompoundTokenDetails details) {
      ...
      switch (first) {
        ...
        case 'U':
          if (details.IsSet((short)StringOptions.AllowsUEscapes)) {
            ...
            if (segment.Length < len + 1) {
              details.Error = string.Format(O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrBadUnEscape, segment.Substring(len + 1), len);// "Invalid unicode escape ({0}), expected {1} hex digits."
              return segment;
            }
        ...
        case 'x':
          if (details.IsSet((short)StringOptions.AllowsXEscapes)) {
            ...
            if (p <= 1) {
              details.Error = O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrBadXEscape; // @"Invalid \x escape, at least one digit expected.";
              return segment;
            }
      ...
      details.Error = string.Format(O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrInvEscape, segment); //"Invalid escape sequence: \{0}"
      return segment; 
    }//method
    #endregion
