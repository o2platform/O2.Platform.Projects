  ...
  public class NumberLiteral : CompoundTerminalBase {
    ...
    protected internal override void InvokeValidateToken(ParsingContext context) {
      if (!IsSet(NumberOptions.AllowLetterAfter)) {
        ...
        if(char.IsLetter(current) || current == '_') {
          context.CurrentToken = context.Source.CreateErrorToken(O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrNoLetterAfterNum); // "Number cannot be followed by a letter." 
        }
      }
    ...
    protected override bool ConvertValue(CompoundTokenDetails details) {
      if (String.IsNullOrEmpty(details.Body)) {
        details.Error = O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrInvNumber;  // "Invalid number.";
        return false;
      }
