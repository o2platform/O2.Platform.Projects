  ...
  public class StringTemplateNode : AstNode {
    ...
    private void ParseSegments(ParsingContext context) {
      ...
      while(true) {
        ...
        if (endTagPos < 0) { 
          context.AddParserError(O2_Misc_Microsoft_MPL_Libs.Irony_Parser.Resources.ErrNoEndTagInEmbExpr, _templateSettings.EndTag);//"No ending tag '{0}' found in embedded expression."
          return;
        }
